...
async def webhook(request: Request, background_tasks: BackgroundTasks):
    ...
    _, data = await max_update_handler.handle_webhook(request)

    if isinstance(data, list):
        for update_item in data:
            await max_update_handler.process_update(
                update_item, request, background_tasks
            )
    else:
        await max_update_handler.process_update(data, request, background_tasks)
