Контекст проекта:
Я разрабатываю коммерчекого бота с ИИ.
Стек: Python (FastAPI), библиотека LangChain.
LLM и Эмбеддинги: ChatGpt, Gemini, GigaChat (через langchain_community.chat_models и GigaChatEmbeddings).
Векторное хранилище: ChromaDB.
Платформа: Мессенджер "MAX" (Россия).
Есть API ключи для ChatGpt, Gemini, GigaChat, GigaChatEmbeddings, SaluteSpeech
Текущее состояние:
...
Чат с поиском в интернет на chatgpt создание и редактирование изображений на openai и gemini
...
Не удаляй существующие коментарии, если не согласен с комментарием то выше напиши свой
